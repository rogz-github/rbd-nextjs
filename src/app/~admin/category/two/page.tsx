...
function SortableCategoryRow({ 
  ...
  category: CategoryTwoData
  onDelete: (category: CategoryTwoData) => void
        ...
        <div className="flex items-center space-x-4">
          <div className="flex-shrink-0">
            <img
              ...
              src={category.psstImg || '/placeholder-product.jpg'}
              alt={category.psstSubcat}
            />
          </div>
          <div>
            <h4 className="text-lg font-medium text-gray-900">{category.psstSubcat}</h4>
            <p className="text-sm text-gray-500">Slug: {category.psstSlug}</p>
            <p className="text-sm text-gray-500">Position: {category.psstPosition}</p>
        ...
        <div className="flex items-center space-x-2">
          ...
          {isSuperAdmin && (
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
              {formatNumber(category.psstTotalProduct)} products
            </span>
          )}
          <button 
            onClick={() => onEdit(category.psstSlug)}
...
function EditCategoryModal({ 
  ...
  const [formData, setFormData] = useState({
    psstImg: category.psstImg,
    psstSubcat: category.psstSubcat,
    seoTitle: category.seoTitle,
    seoDesc: category.seoDesc,
    psstSlug: category.psstSlug
  })
  const [previewUrl, setPreviewUrl] = useState(category.psstImg)
  ...
  useEffect(() => {
    ...
    setPreviewUrl(category.psstImg)
    setErrors({})
  }, [category])
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (result.success) {
        setFormData(prev => ({ ...prev, psstImg: result.url }))
  ...
  const handleUrlChange = (url: string) => {
    setFormData(prev => ({ ...prev, psstImg: url }))
    setPreviewUrl(url)
    if (errors.image) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/category-two', {
        method: 'PUT',
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
      <div className="p-6 border w-full max-w-2xl shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-semibold text-gray-900">Edit Category Two</h3>
          <button
            onClick={onClose}
...
export default function CategoryTwoPage() {
  const { data: session, status } = useSession()
  const router = useRouter()
  const [categories, setCategories] = useState<CategoryTwoData[]>([])
  ...
  const [categoryToDelete, setCategoryToDelete] = useState<CategoryTwoData | null>(null)
  const [isDeleting, setIsDeleting] = useState(false)
  const [isGenerating, setIsGenerating] = useState(false)
  const [showGenerateModal, setShowGenerateModal] = useState(false)
  const [showEditModal, setShowEditModal] = useState(false)
  const [categoryToEdit, setCategoryToEdit] = useState<CategoryTwoData | null>(null)
  ...
  // Handle delete click
  const handleDeleteClick = (category: CategoryTwoData) => {
  ...
  const handleEditClick = (slug: string) => {
    const category = categories.find(cat => cat.psstSlug === slug)
    if (category) {
  ...
  const filteredCategories = categories.filter(category => {
    ...
    const matchesCategoryOne = !selectedCategoryOne || category.psstPcstCat === selectedCategoryOne
    
    // Filter by search term
    const matchesSearch = searchTerm === '' || 
      category.psstSubcat.toLowerCase().includes(searchTerm.toLowerCase()) ||
      category.psstSlug.toLowerCase().includes(searchTerm.toLowerCase()) ||
      category.psstPcstCat.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const generateCategories = async () => {
    ...
    try {
      const response = await fetch('/api/admin/category-two/generate', {
        method: 'POST',
        headers: {
  ...
  const deleteCategory = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await fetch(`/api/admin/category-two/${categoryToDelete.psstSlug}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const filteredOldIndex = filteredCategories.findIndex(cat => cat.psstId === active.id)
    const filteredNewIndex = filteredCategories.findIndex(cat => cat.psstId === over.id)
    ...
    const updatedCategories = categories.map(cat => {
      const updatedFilteredCat = updatedFiltered.find(fc => fc.psstId === cat.psstId)
    ...
    try {
      const response = await fetch('/api/admin/category-two/reorder', {
        credentials: 'include',
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          categories: updatedCategories.map(category => ({
            id: category.psstId,
            position: category.psstPosition
          }))
        })
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Category Two Management</h1>
            <p className="mt-2 text-gray-600">Manage your subcategories</p>
          </div>
            ...
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  ...
                  <dd className="text-lg font-medium text-gray-900">
                    {formatNumber(filteredCategories.reduce((sum, cat) => sum + cat.psstTotalProduct, 0))}
                  </dd>
                  </dl>
                </div>
              </div>
      ...
      <div className="bg-white shadow rounded-lg">
        ...
        {categories.length === 0 ? (
          ...
          >
            <SortableContext
              items={paginatedCategories.map(category => category.psstId)}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="p-6 border w-96 shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-4">Generate Category Two from Products</h3>
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  This will analyze all products and create/update categories based on the <code className="bg-gray-100 px-1 rounded">category_2</code> field.
                </p>
                <div className="mt-4 text-left">
                  <h4 className="text-sm font-medium text-gray-900 mb-2">What will happen:</h4>
                  <ul className="text-sm text-gray-600 space-y-1">
                    <li>• New categories will be created for unique category_2 values</li>
                    ...
                    <li>• Parent category (category_1) will be linked</li>
                  </ul>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="py-4 border w-96 shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
            <div className="text-center">
              ...
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  Are you sure you want to delete the category <strong>{categoryToDelete.psstSubcat}</strong>?
                </p>
                <div className="mt-4 text-left">
