...
async function processBatch(products: any[], batchNumber: number) {
  ...
  // Use a transaction to ensure all products in the batch are committed together
    ...
    while (!result.done) {
      ...
      try {
        ...
        if (products.length >= BATCH_SIZE) {
          ...
          // Log progress
          console.log(`Processed batch ${batchNumber}: ${batchResults.success} success, ${batchResults.failed} failed`)
        }
      } catch (error) {
        totalFailed++
