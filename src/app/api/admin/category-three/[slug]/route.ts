...
export async function GET(
  ...
  try {
    ...
    const categoryResult = await prisma.$queryRaw<any>`
      SELECT * FROM category_three WHERE pssst_slug = ${slug} LIMIT 1
    `
    const category = categoryResult[0]

    if (!category) {
      return NextResponse.json(
        { error: 'Category three not found' },
        { status: 404 }
      )
    }
...
export async function PUT(
  ...
  try {
    ...
    console.error('Error updating category three:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.$executeRaw`
      DELETE FROM category_three WHERE pssst_id = ${category.pssst_id}
    `

    return NextResponse.json({ message: 'Category three deleted successfully' })
  } catch (error) {
    console.error('Error deleting category three:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
