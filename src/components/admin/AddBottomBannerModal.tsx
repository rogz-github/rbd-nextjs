...
interface AddBottomBannerModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}

export function AddBottomBannerModal({ isOpen, onClose, onSuccess }: AddBottomBannerModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create banner image
      const response = await fetch('/api/bottom-banners-simple', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || 'Failed to create banner image')
      }
      ...
      <div className="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full mx-auto transform transition-all duration-300 ease-out">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">Add Bottom Banner Image</h3>
            <button
              onClick={handleClose}
              disabled={loading}
          ...
          <p className="text-sm text-gray-600 mt-1">
            Add a new image to your bottom banner section (max 3 images)
          </p>
        </div>
